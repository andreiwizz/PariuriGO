...
# STIL NOU FUTURIST: Ecran împărțit simetric (Stânga - Verde Închis / Dreapta - Negru Carbon)
st.markdown("""
<style>
    ...
    /* Coloana din Stânga: Zonă tactică (Verde Închis Abisal) */
    ...
    /* Coloana din Dreapta: Zonă Premium Business (Negru Carbon pur) */
    ...
    .vip-card-box { background: #121212 !important; border-radius: 8px !important; padding: 22px !important; margin-bottom: 22px !important; box-shadow: 0 10px 25px rgba(0,0,0,0.6) !important; }
...
with col_stanga_verde:
    ...
    st.write("---")
    meci_ales = st.selectbox("🎯 Alege partida:", list(meciuri_date.keys()))
    m = meciuri_date[meci_ales]
    
    st.markdown(f"""
    <div class="inner-cyber-card" style="border: 1px solid rgba(0, 255, 102, 0.3);">
        <h2 style='text-align:center; color:#00ff66; margin: 4px 0;'>{meci_ales}</h2>
...
with col_dreapta_negru:
    ...
    for pachet, pret, cul, benf in [
        ...
        <div class="vip-card-box" style="border-left: 4px solid {cul};">
            ...
            <h2 style='margin:8px 0; font-size:34px; color:#ffffff; font-weight:800;'>{pret} <span style='font-size:14px; color:#94a3b8;'>/ lună</span></h2>
