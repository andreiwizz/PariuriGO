...
# Aplicare stiluri de fundal și încărcare date din baza.py
...
with col_meciuri:
    ...
    st.markdown('<div style="width:100%; height:420px; overflow:auto; background:rgba(0,0,0,0.8); border-radius:12px; border:1px solid #1a1a1a; padding:10px; margin-bottom: 25px;"><iframe src="https://scorebat.com" frameborder="0" width="100%" height="390px" allowfullscreen allow="autoplay; fullscreen"></iframe></div>', unsafe_allow_html=True)
    st.write("---")
    ...
    st.markdown("<h2 style='text-align:center; color:#ff9900; margin: 5px 0; font-size:32px; font-weight:800;'>" + meci_ales + "</h2>", unsafe_allow_html=True)
    ...
    st.markdown('<div class="stat-row"><div class="stat-left-val" style="color:#ffaa00; font-size:20px;">' + m["p25_gz"] + '</div><div class="stat-center-label">Peste 2.5 goluri</div><div class="stat-right-val" style="color:#ffaa00; font-size:20px;">' + m["p25_os"] + '</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="stat-row"><div class="stat-left-val" style="color:#ffaa00; font-size:20px;">' + m["gg_gz"] + '</div><div class="stat-center-label">Ambele marchează</div><div class="stat-right-val" style="color:#ffaa00; font-size:20px;">' + m["gg_os"] + '</div></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="stat-row"><div class="stat-left-val" style="font-size:14px; color:#a0aec0;">' + m["cor_gz"] + '</div><div class="stat-center-label">Peste 9.5 cornere</div><div class="stat-right-val" style="font-size:14px; color:#ffaa00;">' + m["cor_os"] + '</div></div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    
    st.markdown('<hr style="border-color: #222; margin: 20px 0;">', unsafe_allow_html=True)
    
    # Barele orizontale din imaginea ta redesenate la indigo
...
with col_abonamente:
    ...
    for pachet, pret, cul, benf in [
        ("🟢 PACHET LOW", "40 RON", "#00ff66", ["3 Bilete analizate / săptămână", "Acces grup comunitate chat"]), 
