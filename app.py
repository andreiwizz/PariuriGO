...
def adauga_membru(user, parola):
    c.execute("INSERT OR REPLACE INTO membri VALUES(?,?)", (user, parola))
    conn.commit()

# ADAUGĂ MEMBRII AICI
adauga_membru("andrei","1234")
adauga_membru("client1","7777")
adauga_membru("vip","9999")
...
with col_meciuri:
    ...
    if not vip:
        st.warning("🔒 Loghează-te ca membru VIP pentru statistici și ponturi.")
        st.stop()
    else:
        st.success("👑 Membru VIP conectat")
