...
""")
conn.commit()
c.execute("INSERT OR IGNORE INTO membri VALUES(?,?)",("admin","pariurigo123"))
conn.commit()
...
with st.sidebar:
    st.title("⚽ LOGIN VIP")
    u = st.text_input("Utilizator")
    p = st.text_input("Parolă", type="password")
    if st.button("Conectare"):
        c.execute("SELECT * FROM membri WHERE user=? AND parola=?", (u,p))
        if c.fetchone():
...
if admin:
    ...
    nume = st.text_input("Utilizator nou")
    parola = st.text_input("Parolă nouă")

    if st.button("➕ Adaugă membru"):
        c.execute("INSERT OR REPLACE INTO membri VALUES(?,?)", (nume, parola))
        conn.commit()
        st.success("Membru adăugat!")

    st.subheader("👥 Membri existenți")
    c.execute("SELECT user FROM membri ORDER BY user")

    for row in c.fetchall():
        c1, c2 = st.columns([4,1])
        c1.write("👤 " + row[0])
        if row[0] != "admin":
            if c2.button("Șterge", key=row[0]):
                c.execute("DELETE FROM membri WHERE user=?", (row[0],))
                conn.commit()
                st.rerun()
