...
# Inițializare stare pentru temă (implicit "dark" pentru aspect nocturn premium)
if "tema" not in st.session_state:
    st.session_state.tema = "dark"
...
# Folosim imaginea trimisă de tine pentru fundal
bg_image_url = "https://ibb.co"

# Ajustarea filtrelor și culorilor în funcție de tema selectată
if st.session_state.tema == "dark":
    # Filtru întunecat peste terenul de fotbal pentru lizibilitate maximă noaptea
    ...
    # Filtru luminos peste terenul de fotbal pentru zi
...
# Injectare stiluri CSS cu imaginea ta exactă de teren de fotbal
st.markdown(f"""
<style>
    /* Aplicarea imaginii tale de teren ca fundal fix */
    .stApp {{
    ...
    /* Cardurile cu efect intens de sticlă mată puse peste teren */
    .glass-card {{
        background: {glass_bg};
    ...
    /* Butoanele de sticlă cu animație premium */
    .stButton > button {{
        background: {glass_bg} !important;
        color: {text_principal} !important;
        border: 1px solid {glass_border} !important;
...
st.markdown(f"<p style='color: {text_secundar}; font-size: 16px; font-weight: 500;'>Design personalizat cu fundal teren de fotbal. Mod: {st.session_state.tema.upper()}</p>", unsafe_allow_html=True)
st.markdown("---")

col_meciuri, col_pachete = st.columns()

# 3. Panoul cu meciuri în stil sticlă
with col_meciuri:
...
# 4. Panoul cu Pachetele de Abonament
with col_pachete:
...
# CORECTAT: Am adăugat cifra 2 în paranteză pentru a crea exact două coloane în antet
...
# CORECTAT: Am adăugat cifra 2 și aici pentru a împărți ecranul în două secțiuni egale
...
# Imaginea cu terenul de fotbal trimisă de tine
bg_image_url = "https://ibb.co"
